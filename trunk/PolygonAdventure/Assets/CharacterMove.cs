...
public class CharacterMove : MonoBehaviour 
{
	...
	public void DoMove() 
	{
		...
		if(faceAim==false && turnAroundTimer==0f)
		{
			...
			transform.rotation = RotationUpright( Quaternion.RotateTowards(transform.rotation, moveRotation, rotationSpeed*dt*4f) );
		}
		else
		{
			speedMult = (speedMult*9 + 0.4f)/10f;
			transform.rotation = RotationUpright( Quaternion.RotateTowards(transform.rotation, aimRotation, rotationSpeed*dt*4f) );
		}
